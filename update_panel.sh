...
select_node(){
    ...
    if [ ! -f $public_file ];then
        download_file $public_file https://proxy.yumus.cn/https://download.miku.ci/install/public.sh
    fi

    . $public_file

    download_Url=$NODE_URL
		downloads_Url=https://gh-proxy.org/https://raw.githubusercontent.com/mianfeikejiwl/bt/refs/heads/main
}
...
get_version(){
    ...
    version=$(curl -Ss --connect-timeout 5 -m 2 https://proxy.yumus.cn/https://www.miku.ci/api/panel/get_version)
    if [ "$version" = '' ];then
        version='7.6.0'
    fi
}
...
update_panel(){
    ...
    if [ ! -f /www/server/panel/data/userInfo.json ]; then
    	...
    	mock_phone_prefixes=(130 131 132 133 135 136 137 138 139 150 151 152 155 156 157 158 159 166 171 172 173 175 176 177 178 180 181 182 183 185 186 187 188 189 191 193 195 196 198 199)
    	mock_phone_prefix="${mock_phone_prefixes[$((RANDOM % ${#mock_phone_prefixes[@]}))]}"
    	mock_username="${mock_phone_prefix}$(LC_ALL=C tr -dc '0-9' </dev/urandom | head -c 8)"
    	mock_address="$(hostname -I 2>/dev/null | awk '{print $1}')"
    	if [ -z "${mock_address}" ]; then
    		mock_address="$(ip route get 1.1.1.1 2>/dev/null | awk '{for(i=1;i<=NF;i++) if($i==\"src\") {print $(i+1); exit}}')"
    	fi
    	[ -z "${mock_address}" ] && mock_address="127.0.0.1"
    	mock_access_key="$(openssl rand -base64 48 | LC_ALL=C tr -dc 'A-Za-z0-9' | head -c 32)"
    	mock_secret_key="$(openssl rand -base64 72 | LC_ALL=C tr -dc 'A-Za-z0-9' | head -c 48)"
    	mock_ukey="$(openssl rand -hex 16)"
    	echo "{\"uid\":${mock_uid},\"username\":\"${mock_username}\",\"address\":\"${mock_address}\",\"access_key\":\"${mock_access_key}\",\"secret_key\":\"${mock_secret_key}\",\"ukey\":\"${mock_ukey}\",\"state\":1}" > /www/server/panel/data/userInfo.json
    fi
...
" 2>/dev/null)
    ...
    if [ "$TodayRunTimes" != "N/A" ] && [ "$TotalRunTimes" != "N/A" ]; then
        ...
        echo ""
        echo -e "\033[31m检测到面板 13.0.0，该版本需要 Python 3.13 运行环境。\033[0m"
        echo -e "\033[31m请执行以下命令完成环境升级：bash /www/server/panel/tools.py repair_pyenv\033[0m"
        echo ""
    fi
fi
}
